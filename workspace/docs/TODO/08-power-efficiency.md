...
| GPU Min / Max Frequency | **1550 / 1550 MHz**（min == max；这只是**请求值**，不代表真实执行频率） |
...
| **空闲实测功耗** | **43 W** |
| **短时满载实测** | **171 W** / 1550 MHz（② ALU 探针，秒级） |
| **长时满载实测** ⚠️ | **305 → 330 W（已越过 300 W 上限）** / **92 → 101 °C**（GPU0）、**76 °C**（Mem）（② `ze_peak`，每卡连续 ~20 min，2026-09-22） |

> ⚠️ 注意：**`gt_cur/min/max_freq_mhz` 全为 1550**（min == max）。但这只是**请求值** ——
> 实测硬件会自主降额（见下方更正）。意味着：
> - 好处：**短时**结果重复性极佳（短跑跨卡跨次离散度 ≈ 10⁻⁵）
> - 坏处：`gt_cur/max/min_freq_mhz` 与 `xpu-smi GPU Frequency` **永远不会反映真实降额**，
>   想观察真实活跃频率只能看 `/sys/class/drm/card{0,2}/gt_act_freq_mhz`
> - ⚠️ 但 `gt_act_freq_mhz` **本身噪声极大**、不收敛到标准 P-state
>   （标准态只有 RP0=1550 / RP1=1000 / RPn=200；实测采到 200~1400 乱跳）
>   ⇒ **只能当定性证据**，不能反算性能。降额的**硬证据是温度（101 °C）与功耗（> 300 W）**。
> - 做本测试时需先用 `--frequencyrange` 显式放开，才能扫频率点

> 🔴 **重要更正（2026-09-22）**：本方案此前（以及 `Conclusion/02-compute-peak` 早期版本、
> `Conclusion/03/04`）都写过「**满载仅 171 W，远低于 300 W 上限 ⇒ 单卡永远不会碰上功耗墙**」。
> **该结论已撤回。** `ze_peak` 长时间满载（每卡连续 ~20 min、每项 5×10⁹~10¹⁰ work-item）时
> 实测 **GPU Power 305 W，后续复测进一步采到 330 W**（`xpu-smi stats -d 0`）、
> **温度从 92 °C 升到 101 °C**（101 °C 已逼近 PVC 结温上限），
> 而 `gt_cur/max/min_freq_mhz` 仍读 1550（即"目标频率"未变，是硬件自主降额）。
> ⇒ **热/功耗降额确实存在，只是需要长时间满载才会触发**（性能折扣 ~0.87×）。
> 本测试方案因此多了一个高价值目标：**测「满载时长 → 稳态性能/功耗」曲线**（见 §4 Step 4）。
> ⚠️ 若要同时记录频率曲线，**必须同时记录温度与功耗**，且对 `gt_act_freq_mhz` 作定性说明。
...
| 实测功耗**短时**远低于 Power Limit | 功耗墙未触发；或负载未压满 |
| **短时 171 W 但长时（>10 min）功耗爬到 305~330 W、温度升到 101 °C** | ✅ **已实测确认：长时满载会触发热/功耗降额**（② `ze_peak`，2026-09-22）。引用峰值时必须注明是短时读数；长时稳态性能打 ~0.87 折扣 |
...
2. 频率请求值固定 1550 MHz（min==max）。测频率曲线前要知道这个前提；
   另外 `gt_act_freq_mhz` 噪声很大，**不要把它当精确时钟读数**。
   ⚠️ 但请求值固定只约束"目标频率"：**长时间满载时硬件会自主降额**
   （2026-09-22 实测，温度 101 °C、功耗 305~330 W）—— 所以测功耗时**必须记录时长与温度**。
3. **`--powerlimit` / `--frequencyrange` 需要时间生效**，改完先等稳态再测量。
4. `--memoryecc` 改动可能需要 **reboot/GPU reset**，且影响数据完整性 → **高风险**。
5. **双卡满载按 305~330 W/卡 = 610~660 W 估算** → 确认 PSU 能力，避免触发整机保护。
6. 温度上升会导致频率回落 → 本测试需**同时记录温度**（对照 ①）。
7. `Energy(J)` 指标比 `Power(W)` 采样更适合算平均功耗。
8. 每档配置**至少测 3 次取中位数**，排除瞬态干扰。
9. **恢复配置后必须复测一次**，确认性能回到基线（防止残留状态）。
10. **可直接复用现成负载**：`benchmark/02-compute-peak/ze_peak_src/build/ze_peak`
    是理想的稳态满载源（单卡 ~15 min、稳定 99% 占用），比临时写 kernel 靠谱。
